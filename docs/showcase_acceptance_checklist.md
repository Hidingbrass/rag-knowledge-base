...
| 规则小聊 | 分别发送“你好”“谢谢”“你能做什么” | 返回固定回复、无引用、`retrievalMode=small_talk`，且不调用 FastAPI 或模型 |
| 组合句边界 | 发送“你好，请总结这份文档” | 不误判为小聊，继续进入严格 RAG |
| 开放域路由 | 发送“1+1 等于多少” | 规则未命中后由分类器判断，返回无引用答案并记录 `retrievalMode=open_domain_chat` |
| 企业知识保护 | 发送“我们公司的报销制度是什么” | 即使资料中没有答案也进入 RAG，低相关时拒答，不切换到开放域回答 |
| 分类降级 | 模拟分类超时、无效 JSON 或低置信输出 | 确定性写操作/实时门禁先处理；其余无法安全判断的问题保守进入 RAG |
| 实时查询门禁 | 发送“今天合肥天气怎么样” | 不进入 RAG、不调用模型或工具，记录 `realtime_tool_unavailable` 并说明工具未接入 |
| 写操作门禁 | 发送“帮我删除这份知识库文档” | 不进入 RAG、不调用模型或删除接口，记录 `destructive_action_blocked` 并要求走授权管理入口 |
| 工具缺参澄清 | 模拟分类器返回 `READ_TOOL` 且缺少参数 | 记录 `clarification`，只追问最关键参数，不声称已经执行 |
| RAG 问答 | 前端创建会话并提问 | 返回答案和引用来源 |
| 拒答能力 | 提问资料中没有依据的企业知识问题 | 系统不强行编造，也不切换成无引用的通用模型回答 |
