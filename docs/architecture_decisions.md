...
明确企业知识、低于 0.90、未知标签、无效结构或分类服务异常全部进入严格 RAG；只有高置信非企业意图可以绕开检索。
```
...
当前没有通用工具执行器；除 ADR-017 固定注册的两个工具外，TOOL_CALL 只能给出步骤或追问必要参数，不得声称外部操作已经执行。
...
```text
该阶段建立了独立人工标注意图路由集和自动评测程序；真实 DashScope 结果及新阈值见 ADR-017。
该阶段尚无 ToolRegistry 或真实工具执行器；后续授权范围见 ADR-017。
规则只覆盖高置信模式，剩余问题仍需要分类器和保守 RAG 回退共同处理。
```

## ADR-017：白名单工具执行、写操作二次确认与真实分类校准

决策：

```text
工具执行权属于 Spring Boot 固定注册表，不属于意图分类模型。首批只授权 get_current_weather 只读工具和 delete_knowledge_document 写工具。
天气工具只访问配置中固定的 Open-Meteo 域名；模型不能提供任意 URL。缺少城市时进入澄清，工具失败时返回错误，不回退到 RAG 猜测实时数据。
删除当前文档先把服务端解析出的知识库、文档 ID 和文件名保存为 PENDING 操作，用户以精确确认文本二次确认后才执行；默认 5 分钟失效。
确认时重新校验 JWT 所有者、会话和知识库权限，使用数据库悲观锁保证单次执行，并同步删除业务记录与向量数据。
```

分类校准：

```text
2026-08-12 使用真实 DashScope qwen-flash 运行 21 条人工标注集。intent 准确率 85.71%，四字段全对率 76.19%，企业知识范围召回率 100%，写工具召回率 75%，实时查询召回率 100%。
阈值扫描在保持企业知识 RAG 召回率 100% 的前提下推荐 0.90，因此 FastAPI 和 Spring Boot 默认阈值同时从 0.80 调整为 0.90。
该结果来自小规模人工集和模型自报 confidence，只作为项目初始阈值，不代表生产准确率或严格概率校准。
```

验证边界：

```text
Docker Compose 已按当前源码重建；在线 JWT 接口与真实 Chromium 页面均完成天气查询、待确认展示、错误确认不执行、正确确认删除、跨用户 403、重复确认幂等和 Qdrant 清理验收。
授权范围仍然只有两个固定工具；任意工具、任意 URL、修改、发送和发布等通用写操作继续阻断。
```
