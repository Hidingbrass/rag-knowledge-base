...
我在 Spring Boot 正式会话入口先增加确定性策略路由：纯小聊直接回复；删除、修改、发送等写操作在模型前阻断；明确企业知识固定进入 RAG；天气、汇率等实时查询在没有授权工具时明确说明不可用。只有这些规则都未命中后，才调用独立轻量模型判断知识问答、工具调用、澄清和开放域聊天。

这里不能简单判断是否包含“你好”。“你好，请总结这份资料”和“你好像没有回答问题”都包含相同字符，但有真实语义。当前资料或企业内部事实继续经过权限、检索、阈值、引用和拒答；写操作不能因为涉及知识库就进入 RAG，也不能仅凭模型标签执行。当前没有通用工具执行器，因此写操作直接阻断，实时读取工具不可用时明确说明，只有缺少参数的读取请求才进入澄清。
```
