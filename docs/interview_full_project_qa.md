...
我在 Spring Boot 正式会话入口先增加确定性策略路由：纯小聊直接回复；当前天气调用固定白名单只读工具；删除当前文档先暂存并要求二次确认；其他写操作继续阻断；明确企业知识固定进入 RAG。只有这些规则都未命中后，才调用独立轻量模型判断知识问答、工具调用、澄清和开放域聊天。

这里不能简单判断是否包含“你好”。“你好，请总结这份资料”和“你好像没有回答问题”都包含相同字符，但有真实语义。当前资料或企业内部事实继续经过权限、检索、阈值、引用和拒答；工具执行不能因为模型输出了名称就放行。当前注册表只授权天气读取和当前文档删除，工具名、参数、权限、确认、幂等和审计都由 Spring Boot 控制；其余实时读取和写操作继续明确阻断。
```
