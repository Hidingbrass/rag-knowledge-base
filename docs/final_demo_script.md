...
3. 输入一个资料内问题，查看 RAG 流式答案和引用来源。
4. 输入一个资料外事实问题，确认系统仍严格拒答。
5. 刷新页面，确认三类 USER / ASSISTANT 消息都能恢复。
```
...
未命中直答规则的问题才调用 FastAPI 的 Rerank RAG 接口；包含实际问题的混合消息不会被问候关键词误截断。
```
...
问题 -> 权限校验 -> 确定性规则路由
  -> 纯小聊：确定性回复 -> 保存消息
  -> 知识问题：Embedding -> Qdrant 召回 -> qwen3-rerank 精排 -> 阈值拒答 -> Prompt -> 通义千问 -> 引用来源
```

这里不是“检索失败后切换通用聊天”：只有事先定义的纯小聊会被本地规则命中，任何实质问题和组合句仍进入严格 RAG。
