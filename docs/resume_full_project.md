# 完整项目简历材料：企业智能知识库 RAG 系统

## 简历项目名称

Spring Boot + FastAPI 企业智能知识库 RAG 问答系统

## 一句话描述

基于 Spring Boot、FastAPI、MySQL、Qdrant 和通义千问构建企业知识库问答系统，支持 PDF 文档入库、RAG 问答、Rerank 重排、引用溯源、拒答、知识库权限控制、聊天会话持久化和前端演示。
...
```text
基于 Spring Boot + FastAPI + MySQL + Qdrant + 通义千问实现企业知识库 RAG 问答系统。Spring Boot 负责知识库、文档状态、权限校验、聊天会话和消息持久化；FastAPI 负责 PDF 解析、Chunk 切分、Embedding、向量检索、qwen3-rerank 和大模型问答；MySQL 保存业务数据，Qdrant 保存文本片段向量。项目支持指定文档问答、引用来源、无依据拒答、重复文档检测、前端演示和自动化评测。
```
...
```text
FastAPI pytest：67 passed
Spring Boot Maven test：22 passed
```
...
3. 系统集成能力：Spring Boot 调 FastAPI，业务数据和向量数据分离。
...
为了减少幻觉，我实现了引用来源、无依据拒答和 qwen3-rerank 重排，并建立了 30 条评测集，对回答通过率、拒答准确率、引用有效率、引用支持率和 Rerank 参数进行对比。项目还提供了前端演示页和自动化测试，方便完整展示从文档上传到问答和权限控制的流程。
```
