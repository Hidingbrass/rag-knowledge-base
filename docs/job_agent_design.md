...
这样设计的好处是边界清晰：Python FastAPI 负责 AI 能力，Java Spring Boot 负责业务接口、数据库和历史记录。后续如果要加简历优化、面试题生成、分析结果分区展示、收藏岗位，都可以在这个结构上继续扩展。
```
...
5. 前端分区展示 strengths、risks、suggestions 和 interview_questions。
```
