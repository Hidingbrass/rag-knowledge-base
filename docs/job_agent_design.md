...
这样设计的好处是边界清晰：Python FastAPI 负责 AI 能力，Java Spring Boot 负责业务接口、数据库和历史记录。后续如果要加简历优化、面试题生成、历史详情、收藏岗位，都可以在这个结构上继续扩展。
```
...
```text
1. GET /api/job-agent/tasks/{taskId} 查询单条历史详情。
2. DELETE /api/job-agent/tasks/{taskId} 删除历史记录。
3. 简历结构化解析：提取技能、项目、年限、学历等。
4. JD 结构化解析：提取岗位技能、职责、加分项。
5. 简历优化建议：根据 JD 生成更匹配的项目表述。
6. 面试准备包：生成自我介绍、项目讲解、常见追问。
7. 前端分区展示 strengths、risks、suggestions 和 interview_questions。
```
