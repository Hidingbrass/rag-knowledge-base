...
## 12. 2026-08-12：真实意图评测与授权工具闭环

实现：

- 使用真实 DashScope `qwen-flash` 跑完 21 条人工标注意图集，新增阈值扫描并把 FastAPI、Spring Boot 默认置信阈值从 `0.80` 同步调整为 `0.90`。
- 新增固定白名单工具注册表。`get_current_weather` 只访问配置的 Open-Meteo 地址；缺少城市时澄清，失败时不回退到 RAG 猜测。
- `delete_knowledge_document` 仅处理当前已选文档。服务端锁定参数并持久化 `PENDING` 操作，默认 5 分钟内由原 JWT 用户二次确认；执行时重新校验权限、悲观锁和状态。
- Flyway V8 新增 `tool_action` 表；前端显示待确认卡片，确认后同步刷新消息和文档列表。路由和执行状态继续写入聊天审计字段。
- 修复真实验收发现的流式 JWT 问题：初始请求通过鉴权后只放行 Servlet 内部 `ASYNC` 分派，避免已提交 NDJSON 响应被二次安全检查误判为匿名。

验证：

- 真实分类结果：四字段全对 `16/21`（`76.19%`），intent 准确率 `85.71%`，企业知识范围召回率 `100%`，写工具召回率 `75%`，实时查询召回率 `100%`。原始报告保存在 `app/evaluation/evaluation_results/intent_routing_dashscope_20260812.json`。
- Docker Compose 按当前源码重建并启动，Spring Boot、FastAPI、MySQL、Redis、Qdrant 在线冒烟检查通过。
- 在线 JWT 与 Chromium 验收覆盖天气查询、错误确认不执行、待确认 UI、正确确认删除、跨用户确认 `403`、重复确认幂等；删除后业务文档数和 Qdrant 对应点数均为 `0`。
- 验收使用的知识库、文档、向量、会话、工具记录和两个临时账号已清理，数据库查询剩余数为 `0`。
- 完整提交门禁通过：Vue `12 passed`、FastAPI `179 passed`、Spring Boot `200 passed`，前端生产构建、Markdown 链接、密钥扫描、空白检查和 Compose 配置检查均通过。
- 21 条人工集规模仍小，模型置信度不是严格校准概率；主要误差集中在澄清意图和部分工具请求，不能外推为生产准确率。
