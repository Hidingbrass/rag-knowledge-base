...
一个基于 Spring Boot、FastAPI、Qdrant、MySQL 和通义千问的企业知识库 RAG 学习项目。项目从底层实现 PDF 解析、文本切分、Embedding 入库、向量检索、RAG 问答、引用来源、拒答、Rerank、质量评测、知识库权限、聊天会话和前端演示页，用于理解企业知识库问答系统的核心链路。
...
- 前端演示页支持上传 PDF、创建会话、提问、查看引用来源和权限验证
...
Spring Boot：已接入 MySQL/JPA、知识库权限、文档状态、重复上传检测、聊天会话、消息持久化和静态演示页，当前 Maven 测试 22 passed
...
```text
前端演示页: http://127.0.0.1:8080/index.html
...
- 继续整理 README、简历材料和面试讲解，统一成“Spring Boot + FastAPI 企业知识库”项目口径。
- 补充 GitHub 提交前检查清单，确认 `.env`、构建产物和本地数据不会提交。
- 进入求职辅助 Agent 模块：简历解析、JD 解析、匹配度分析、差距建议和面试题生成。
...
3. 求职辅助 Agent：实现简历解析、JD 解析、匹配度分析、差距建议和面试题生成。
