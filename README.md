...
| 场景 | 用户可以做什么 | 核心能力 |
| --- | --- | --- |
| AI 学习资料库 | 上传八股文、课程讲义、技术文档，基于资料连续提问 | 多格式解析、混合检索、Rerank、拒答、引用来源、流式输出 |
...
1. 对问题执行安全检查和向量化。
2. 并行获取 Dense 与 Sparse 候选。
3. 使用 RRF 融合，并通过 Rerank 精排和文档级去重。
4. 相关度不足时拒答，避免脱离资料自由发挥。
5. Prompt 约束模型只基于上下文回答。
6. 校验引用来源，以流式事件持续返回状态、来源和答案片段。
...
- Spring Boot 业务、权限和接口：121 个测试。
- Vue 流式解析与状态交互：10 个测试。
- 合计：298 个自动化测试。
