...
- Spring Boot 先执行确定性小聊、写操作阻断、企业知识保护和实时查询门禁；只有剩余问题才调用轻量分类模型。
- 写操作不进入 RAG 且不会执行；未接入实时工具时透明说明不可用；明确企业知识和无法安全判断的分类故障仍保守进入 RAG。
...
1. Spring Boot 校验会话、资料库和所选文档权限，再按“确定性小聊 → 写操作门禁 → 企业知识保护 → 实时查询门禁 → 轻量分类器”顺序路由。
2. 纯问候、感谢和能力询问使用确定性模板直接回复，同时正常保存用户与助手消息，不调用模型服务。
3. 删除、修改、发送等外部写请求由 Spring Boot 确定性阻断；明确天气、汇率等实时问题在没有授权工具时直接说明不可用，两者都不进入 RAG。
4. 明确指向当前资料、知识库或企业内部事实的消息固定进入 RAG；其他消息由 `QWEN_INTENT_MODEL` 输出意图、知识范围、操作类型、时效性、缺失参数和确认要求。
5. 分类低于 `0.80`、输出无效、服务异常或出现企业知识标记时强制回到 RAG；分类器识别出的写操作即使涉及企业资料也优先阻断。
6. RAG 并行获取 Dense 与 Sparse 候选，使用 RRF 融合，再通过 Rerank 精排、阈值拒答和引用校验。
7. 仅高置信非企业问题使用普通模型回答；路由结果以结构化 JSON 写入消息审计字段，包含来源、原因、置信度和风险维度。
8. 所有分支复用会话权限、MySQL 消息记录和 NDJSON 协议；实际模型调用继续进入成功率、延迟、Token 与成本审计。
...
| `INTENT_CLASSIFIER_MIN_CONFIDENCE` | 非 RAG 路由最低置信度，默认 `0.80`；低于阈值回到 RAG |
...
报告包含字段准确率、企业知识范围召回率、写操作召回率、实时查询召回率和意图混淆矩阵。当前只验证了数据集与评测程序本身，尚未运行真实 DashScope 全集，因此没有可声明的分类准确率或最佳阈值。
...
| 如何处理写操作和实时问题 | 写操作在模型前阻断；实时工具未接入时透明说明不可用，不用 RAG 猜测结果 |
